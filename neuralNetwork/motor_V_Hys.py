...
train_data = pd.concat(
    [train_data, pd.read_csv(PATH / f"idiq{TRAIN_FILE}").drop(columns="Unnamed: 0")],
    axis=1
)
train_data["speed"] = pd.read_csv(PATH / f"speed{TRAIN_FILE}")["N"]
train_data = pd.concat(
    [train_data, pd.read_csv(PATH / f"xgeom{TRAIN_FILE}").drop(columns="Unnamed: 0")],
    axis=1
)
...
test_data = pd.concat(
    [test_data, pd.read_csv(PATH / f"idiq{TEST_FILE}").drop(columns="Unnamed: 0")],
    axis=1
)
test_data["speed"] = pd.read_csv(PATH / f"speed{TEST_FILE}")["N"]
test_data = pd.concat(
    [test_data, pd.read_csv(PATH / f"xgeom{TEST_FILE}").drop(columns="Unnamed: 0")],
    axis=1
)
...
class mixedModel(nn.Module):
    def __init__(
        ...
        # PROJEÇÃO: Nabla → espaço do V
        self.input_adapter = nn.Sequential(
            nn.Linear(input_dim_model2, input_dim_V),
            nn.ReLU()
        )

        # Backbone transferido INTACTO
        self.transL = transL(
...
def register_csv(contents, info):
    ...
    SAVE_DIR = BASE_DIR / ".." / "transL_results"
    SAVE_DIR.mkdir(parents=True, exist_ok=True)
    SAVE_PATH = SAVE_DIR / "motor_Nabla_Hys_TransL_info.csv"
...
input_dim_nabla = train_data.drop(columns=target).shape[1]
...
assert TRANS_WEIGHTS.exists(), f"Pesos não encontrados: {TRANS_WEIGHTS}"

# número de entradas do motor V (vem do peso salvo)
state_tmp = torch.load(TRANS_WEIGHTS, map_location="cpu")
INPUT_DIM_V = state_tmp["linear.0.weight"].shape[1]
...
for hn in head_neurons:
    for hl in head_layers:
        for lr in learning_rates:
            ...
            model = mixedModel(
                input_dim_model2=input_dim_nabla,
            ...
            # ==== CARREGA 100% DOS PESOS DO MOTOR V ====
            state = torch.load(TRANS_WEIGHTS, map_location="cpu")
            new_state = {k.replace("linear.", "net."): v for k, v in state.items()}
            model.transL.load_state_dict(new_state, strict=True)

            # adapter e head treináveis
            for p in model.input_adapter.parameters():
                p.requires_grad = True
            for p in model.head.parameters():
                p.requires_grad = True

            # backbone começa congelado
            for p in model.transL.parameters():
